...
def perform_enhancement(addr_in, mode, shps):

    addr_out = addr_in.replace('_e.wav', '.noisyMchSTFT.npy')
    print('%s' % addr_in)
    print(' -> %s' % addr_out)
    ...
    #                                   length, shps.ratio)
    enhance_stft = mask * mag_noisy * np.exp(1j*pha_noisy)
    enhance_stft = enhance_stft.astype(np.complex)
    enhanced_out = addr_out.replace('noisy_mic0', 'noisymchstft')

    return enhance_stft, enhanced_out


stft_mic0 = []
stft_name = []
for (path, dir, files) in os.walk("../data/speech/test/noisy_mic0"):
    for data0 in files:
        ext = os.path.splitext(data0)[-1]
        if ext == '.wav':
            addr_input = "%s/%s" % (path, data0)
            addr_new = path.replace('noisy_mic0', 'noisymchstft')
            makedirs(addr_new, exist_ok=True)

        enhance_stft, enhanced_out = perform_enhancement(addr_input, mode, shps)
        stft_mic0.append(enhance_stft)
        stft_name.append(enhanced_out)
        # makedirs(enhanced_out, exist_ok=True)

stft_mic1 = []
for (path, dir, files) in os.walk("../data/speech/test/noisy_mic1"):
    for data1 in files:
        ext = os.path.splitext(data1)[-1]
        if ext == '.wav':
            addr_input = "%s/%s" % (path, data1)
            # addr_new = path.replace('noisy_mic1', 'mic1_stft')
            # makedirs(addr_new, exist_ok=True)

        enhance_stft, _ = perform_enhancement(addr_input, mode, shps)
        stft_mic1.append(enhance_stft)

for n in range(len(stft_mic0)):
    stft_out = np.zeros((stft_mic0[n].shape[0], 2, stft_mic0[n].shape[1]), dtype=np.complex)
    stft_out[:,0,:] = stft_mic0[n]
    stft_out[:,1,:] = stft_mic1[n]

    np.save(stft_name[n], stft_out)
