...
def main():
    if len(sys.argv) != 4:
        ...
        print('python extract_features_lg.py [type_name] [dir_name] [output_name]')
        exit()

    # Initialize setup.
    feature_type, wav_dir_name, addr_feature = sys.argv[1:] # make parameter using for command
    # feature_type = 'irm'
    # wav_dir_name = '../data_lg/speech/train'
    # addr_feature = 'feature_data/train_ref_irm_lg'
    ...
    addres = []
    for n in range(9):
        addr = scan_directroy('%s/noisy{}'.format(n) % wav_dir_name, n)
        addres.extend(addr)
    #addres = scan_directory1('%s/noisy' % wav_dir_name)
    
    # Get feature data from address of database.
    data = read_data_from_addr(addres, feature_type, shps)
...
def scan_directroy(dir_name, n):
    ...
    for subdir, dirs, files in os.walk(dir_name):
        for file in files:
            ...
            if filepath.endswith(".wav"):
                addr_noisy = filepath
                addr_clean = filepath.replace('/noisy{}/'.format(n), '/clean/')
                addr.append([addr_noisy, addr_clean])
                print(addr_noisy, addr_clean)
    return addr


# 위에서 return 해준 noisy 와 clean 의 address 로 read
def read_data_from_addr(addres, feature_type, shps):
    data = []
    for i in range(len(addres)):
        addr_noisy = addres[i][0]
        addr_clean = addres[i][1]
        print("    [%d/%d] %s" % (i + 1, len(addres), addr_noisy))

        # Read wave files.
        fs, audio1 = wav.read(addr_noisy)  # read noisy data
        wav_noisy = audio1.astype(np.float16) / (2 ** 15)  # change data type
        fs, audio2 = wav.read(addr_clean)  # read clean data
        wav_clean = audio2.astype(np.float16) / (2 ** 15)
