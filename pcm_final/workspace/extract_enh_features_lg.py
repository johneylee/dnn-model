...
def main():
    ...
    # addr_list = scan_directory('%s/noisy' % wav_dir_name)  # for use clean / noisy
    addr_list = []  # for use clean / noisy0 ~ noisy8
    for n in range(5):
        addr = scan_directory_many('%s/noisy{}'.format(n) % wav_dir_name, n)
...
def scan_directory_many(dir_name, n):
    """Scan directory and save address of clean/noisy wav data.
    ...
    for subdir, dirs, files in os.walk(dir_name):
        for file in files:
            ...
            if filepath.endswith(".wav"):
                addr_noisy = filepath
                addr_clean = filepath.replace('/noisy{}/'.format(n), '/clean{}/'.format(n))
                addr_list.append([addr_noisy, addr_clean])
    return addr_list
