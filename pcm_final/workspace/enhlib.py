...
def define_real(fc_out, opt='sigmoid'):
    if opt in ['sigmoid, sigm']:
        est_mask_real = tf.nn.sigmoid(fc_out)
    elif opt in ['hyperbolic_tangent', 'tanh']:
        est_mask_real = tf.nn.tanh(fc_out)
    elif opt in ['relu', 'ReLU']:
        est_mask_real = tf.nn.relu(fc_out)
    else:
        est_mask_real = fc_out

    return est_mask_real


def define_real_imag(fc_out):
    dim = 257
    est_mask_real = tf.slice(fc_out, [0, 0, 0 * dim], [-1, -1, dim])
    est_mask_imag = tf.slice(fc_out, [0, 0, 1 * dim], [-1, -1, dim])
    est_mask_real = tf.maximum(tf.minimum(est_mask_real, 20.0), -20.0)
    est_mask_imag = tf.maximum(tf.minimum(est_mask_imag, 20.0), -20.0)

    return est_mask_real, est_mask_imag


def define_pcm(fc_out):
    dim = 257
    ...
    fc_out4 = tf.slice(fc_out, [0, 0, 3 * dim], [-1, -1, dim])
...
def define_obj_ma(ref, est_mask_mag, rnn_mask):
    obj = masked_mse(est_mask_mag / ref['irm'].sig,
                     ref['irm'].data_vnorm, rnn_mask)
    return obj


def define_obj_msa(ref, est_mag, rnn_mask):
    # MSA
    obj = masked_mse(est_mag / ref['mag_clean'].sig,
                     ref['mag_clean'].data_vnorm, rnn_mask)
    return obj


def define_obj_tdr_old(ref, est_spec_real, est_spec_imag,
                       dim, seq_length, rnn_mask):
    # TDR without normalization
    ...
    est_frm_hann_clean_vnorm \
        = est_frm_hann_clean / ref['frm_hann_clean'].sig
    obj = masked_mse(est_frm_hann_clean_vnorm,
                     ref['frm_hann_clean'].data_vnorm,
                     rnn_mask)
    return obj


def define_obj_tdr(ref, est_spec_real, est_spec_imag,
                   dim, seq_length, rnn_mask):
    ...
    est_frm_hann_norm_clean = est_frm_hann_clean / sig_max
    est_frm_hann_norm_clean_vnorm \
        = est_frm_hann_norm_clean / ref['frm_hann_norm_clean'].sig
    obj = masked_mse(est_frm_hann_norm_clean_vnorm,
                     ref['frm_hann_norm_clean'].data_vnorm,
                     rnn_mask)
    return obj


def define_obj_etdr_old(ref, est_spec_real, est_spec_imag,
                        dim, seq_length, rnn_mask):
    # TDR without normalization
    ...
    est_frm_rect_clean \
        = overlap_and_add(est_frm_hann_clean, batch_size,
                          numfrm_max, dim)
    est_frm_rect_clean_vnorm \
        = est_frm_rect_clean / ref['frm_rect_clean'].sig
    obj = masked_mse(est_frm_rect_clean_vnorm,
                     ref['frm_rect_clean'].data_vnorm,
                     rnn_mask)
    return obj
...
def define_obj_etdr(ref, est_spec_real, est_spec_imag,
    ...
    est_frm_rect_norm_clean_vnorm \
        = est_frm_rect_norm_clean / ref['frm_rect_norm_clean'].sig
    obj = masked_mse(est_frm_rect_norm_clean_vnorm,
                     ref['frm_rect_norm_clean'].data_vnorm,
                     rnn_mask)
    return obj
...
def define_obj_sc(ref, est_mask_mag, noisy_mag, noisy_pha,
    ...
    est_frm_win_fixed = tf.matmul(est_spec_real_fixed,
                                  irdftmat_real) - tf.matmul(
        est_spec_imag_fixed, irdftmat_imag)
    est_frm_fixed = overlap_and_add_full(est_frm_win_fixed, batch_size,
                                         numfrm_max, dim)
    est_respec_real_fixed = tf.matmul(est_frm_fixed * wa, rdftmat_real)
    est_respec_imag_fixed = tf.matmul(est_frm_fixed * wa, rdftmat_imag)
...
def convert_magpha2concatenated_spec(mag, pha):
    """Graph definition for converting
       magnitude and phase to complex, real, and imaginary spectrum.
    """
...
def convert_magpha2concatenated_spec_only(mag, pha):
    """Graph definition for converting
       magnitude and phase to complex spectrum.
    """
    spec_real = mag * tf.cos(pha)
    spec_imag = mag * tf.sin(pha)
    spec_concat = tf.concat([spec_real, spec_imag], 2)
...
def overlap_and_add(time_signal, batch_size, numfrms, dim):
    ...
    freq_l3 = eye_freq[:, :, 384:512]
    freq_l2 = eye_freq[:, :, 256:384]
    freq_l1 = eye_freq[:, :, 128:256]
    freq_l0 = eye_freq[:, :, 0:128]
