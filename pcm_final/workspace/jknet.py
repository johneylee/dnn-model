...
class JKNetwork:
    ...
    def define_layers(self):
        ...
        self.rnn = []
        if self.hps.rnn_type == 'lstmblockfused':
            for idx in range(num_rnn_layers):
                layer \
                    = rnn.LSTMBlockFusedCell(num_units=num_lstm_cells,
                                             use_peephole=True,
                                             name='lstm_' + str(idx) + '_fw')
                self.rnn.append(layer)

        elif self.hps.rnn_type == 'blstmblockfused':
            for idx in range(num_rnn_layers):
                sublayer_fw \
                    = rnn.LSTMBlockFusedCell(num_units=num_lstm_cells,
                                             use_peephole=True,
    ...
    def feed_rnn_layers(self, data_inp, length, num_rnn_layers,
        ...
        if self.hps.rnn_type == 'lstmblockfused':
            h_lstm_tr = data_inp_tr
            for idx in range(num_rnn_layers):
                h_lstm_tr, _ \
                    = self.rnn[idx](inputs=h_lstm_tr, sequence_length=length,
                                    dtype=tf.float32)
            # Transpose tensor
            out = tf.transpose(h_lstm_tr, [1, 0, 2])
    ...
    def define_graph(self):
        ...
        if structure in ['pcm', 'pcm_pha']:
            dim_out = dim * 4
        elif structure in ['real+imag']:
            dim_out = dim * 2
        else:
            dim_out = dim
        ...
        if structure == 'real':
        ...
        elif structure == 'real+imag':
            est_mask_real, est_mask_imag \
                = enh.define_real_imag(fc_out)
            est_spec_real, est_spec_imag \
                = enh.complex_domain_tf_masking(est_mask_real, est_mask_imag,
                                                noisy_mag, noisy_pha)

        elif structure == 'pcm':
            est_mag_warp_clean_vnorm, est_mag_warp_noise_vnorm, \
            est_cos_xn, est_cos_xy, est_sin_xy \
                = enh.define_pcm(fc_out)
            ...
            fc_h1_pha \
                = tf.contrib.layers.fully_connected(rnn_out_pha, 512,
                                                    activation_fn=tf.tanh)
            est_pha_xy \
                = tf.contrib.layers.fully_connected(fc_h1_pha, 257,
                                                    activation_fn=None)
        ...
        for idx in range(len(obj_mode)):
            if obj_mode[idx] == 'ma':
                obj_ma = enh.define_obj_ma(ref, est_mask_real, rnn_mask)
                obj += obj_weight[idx] * obj_ma

            elif obj_mode[idx] == 'msa':
                obj_msa = enh.define_obj_msa(ref, est_mag, rnn_mask)
                obj += obj_weight[idx] * obj_msa

            elif obj_mode[idx] == 'tdr_old':
                obj_tdr = enh.define_obj_tdr_old(ref, est_spec_real,
                                                 est_spec_imag,
                                                 dim, seq_length, rnn_mask)
                obj += obj_weight[idx] * obj_tdr

            elif obj_mode[idx] == 'tdr':
                obj_tdr = enh.define_obj_tdr(ref, est_spec_real,
                                             est_spec_imag,
                                             dim, seq_length, rnn_mask)
                obj += obj_weight[idx] * obj_tdr

            elif obj_mode[idx] == 'etdr_old':
                obj_etdr = enh.define_obj_etdr_old(ref, est_spec_real,
                                                   est_spec_imag,
                                                   dim, seq_length, rnn_mask)
                obj += obj_weight[idx] * obj_etdr

            elif obj_mode[idx] == 'etdr':
        ...
        self.err1 = obj
        if 'tdr_old' in obj_mode:
            self.err2 = obj_tdr
        elif 'tdr' in obj_mode:
            self.err2 = obj_tdr
        elif 'etdr_old' in obj_mode:
            self.err2 = obj_etdr
        elif 'etdr' in obj_mode:
            self.err2 = obj_etdr
        else:
            self.err2 = obj
        # Define optimizer.
    ...
    def train(self, sess, data_pipeline_train, data_pipeline_devel):
        ...
        for i in range(self.hps.max_epochs + 1):
            ...
            # Print progress.
            print('step %d: %.6f %.6f %.6f (%.0f sec)'
                  % (i, loss_train, loss_devel, err_devel,
                     time.time() - start_time))
            fp.write('step %d: %.6f %.6f %.6f (%.0f sec)\n'
                     % (i, loss_train, loss_devel, err_devel,
                        time.time() - start_time))
            # Save models.
