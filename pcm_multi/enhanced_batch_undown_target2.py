...
def main():
    ...
    model_name = '8/opt'
    noisy_dir = '../data/multi_tar2/KT_TEST/noisy_nomalize'
    #mode = '../../graduate_8/att_2dconv_13'
    mode = '../graduate_8/KT_norm_result'
    # Initialize pairameters
    ...
    for (path, dir, files) in os.walk(noisy_dir):
        for data in files:
            ...
            if ext == '.wav':
                ...
                addr_new = path.replace('noisy', mode)
            ...
            enhanced = net.run_decode_batch_step_origin(sess, buf,
                                                 shps.window_analysis,
                                                 shps.window_synthesis,
                                                 len_wav, shps.ratio)

            #addr_out = addr_input.replace('.wav', '_' + mode.split('/')[-1] + '.wav')
            enhanced_out = addr_input.replace('noisy', mode)
            print('%s' % addr_input)
