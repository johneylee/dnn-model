#!/usr/bin/python

"""
Speech enhancement neural network libraries
It contains functions of network using tensorflow.
Functions include define parameters of network, save and load network model, 
define RNN/fully connected layer, feed layers, define speech enhancement 
network and train/evaluate network.
"""
import os
import numpy as np
import ssplib as ssp
import tensorflow as tf
tf.contrib.rnn
import scipy.io.wavfile as wav
import matplotlib.pyplot as plt
import pdb

class SpeechEnhancementNetwork:
    """Define class of the speech enhancement network for decoding.

    Members:
        hps: hyperparameters of network
        decode: output of network
        err: mean square of estimated data and reference
        optimization: optimizer of network
        data_input: input data of network
        data_reference: reference data of network
        blstm_size: size of BLSTM layer
        length: sequence length vector (the number of frame of each uttrence)
        mask: Binary tensor mask whose components are 0 for padded data and 1 
              for not padded data (Auxiliary tensor for not calculating mse 
              for zero padded data)
        saver_import: tensorflow saver for loading
        saver_export: tensorflow saver for saving
        rnn: RNN layer
        fc: fully connected layer
        mu_input: mean of input data
        mu_reference: mean of reference data
        sig_input: standard deviation of input data
        sig_reference: standard deviation of reference data
        total_params: the total number of parameters of network
        dir_to_save: directory name for save
    """

    def __init__(self):
        print('***********************************************************')
        print('*    Python library for BLSTM-based speech enhancement    *')
        print('*                        using Google\'s TensorFlow API    *')
        print('***********************************************************')
        self.decode = None
        self.attout_r = None
        self.attout_i = None
        self.pcmout_r = None
        self.pcmout_i = None
        self.conv1out_r_1 = None
        self.conv1out_i_1 = None
        self.conv1out_r_2 = None
        self.conv1out_i_2 = None
        self.data_input = None
        self.data_reference = None
        self.dim_input = None
        self.dim_reference = None
        self.batch_size = None
        self.length = None
        self.saver_import = None
        self.mu_input = None
        self.mu_reference = None
        self.sig_input = None
        self.sig_reference = None
        return

    @staticmethod
    def check_model_file_existence(model_name):
        if os.path.isfile('%s/opt.ckpt.meta' % model_name) is False:
            print("[Error] There is no model file '%s'." % model_name)
            exit()
        if os.path.isfile('%s/mu_inp.npy' % model_name) is False:
            print("[Error] There is no mu_inp file '%s'." % model_name)
            exit()
        if os.path.isfile('%s/mu_ref.npy' % model_name) is False:
            print("[Error] There is no mu_refer file '%s'." % model_name)
            exit()
        if os.path.isfile('%s/sig_inp.npy' % model_name) is False:
            print("[Error] There is no sig_inp file '%s'." % model_name)
            exit()
        if os.path.isfile('%s/sig_ref.npy' % model_name) is False:
            print("[Error] There is no sig_refer file '%s'." % model_name)
            exit()

    def import_model(self, sess, model_name):

        self.check_model_file_existence(model_name)
        addr_model = '%s/opt.ckpt' % model_name
        self.saver_import \
            = tf.train.import_meta_graph(addr_model + '.meta',
                                         clear_devices=True)
        self.saver_import.restore(sess, addr_model)
        self.decode = tf.get_collection("decode")[0]
        self.attout_r = tf.get_collection("att_r")[0]
        self.attout_i = tf.get_collection("att_i")[0]
        self.pcmout_r = tf.get_collection("pcmout_r")[0]
        self.pcmout_i = tf.get_collection("pcmout_i")[0]
        self.conv1out_r_1 = tf.get_collection("conv1out_r_1")[0]
        self.conv1out_i_1 = tf.get_collection("conv1out_i_1")[0]
        self.conv1out_r_2 = tf.get_collection("conv1out_r_2")[0]
        self.conv1out_i_2 = tf.get_collection("conv1out_i_2")[0]
        self.data_input = tf.get_collection('data_inp')[0]
        self.data_reference = tf.get_collection('data_ref')[0]
        self.mu_input = np.load('./%s/mu_inp.npy' % model_name)
        self.sig_input = np.load('./%s/sig_inp.npy' % model_name)
        self.mu_reference = np.load('./%s/mu_ref.npy' % model_name)
        self.sig_reference = np.load('./%s/sig_ref.npy' % model_name)

    def import_model2(self, sess, model_name):

        self.check_model_file_existence(model_name)
        addr_model = '%s/opt.ckpt' % model_name
        self.saver_import \
            = tf.train.import_meta_graph(addr_model + '.meta',
                                         clear_devices=True)
        self.saver_import.restore(sess, addr_model)
        self.decode = tf.get_collection("decode")[0]
        self.data_input = tf.get_collection('data_inp')[0]
        # self.batch_prob = tf.get_collection('batch_prob')
        self.data_reference = tf.get_collection('data_ref')[0]
        self.mu_input = np.load('./%s/mu_inp.npy' % model_name)
        self.sig_input = np.load('./%s/sig_inp.npy' % model_name)
        self.mu_reference = np.load('./%s/mu_ref.npy' % model_name)
        self.sig_reference = np.load('./%s/sig_ref.npy' % model_name)
        # self.bnorm = tf.get_collection('Placeholder_1')
        self.bnorm = tf.get_default_graph().get_tensor_by_name('Placeholder_1:0')

    def run_decode_batch_step_origin(self, sess, buf,
                              window_analysis,
                              window_synthesis,
                              length_wav, ratio):

        seqlen = self.get_length_np(buf)
        inp, mask = self.make_data_for_batch(buf, seqlen)
        batch_size = inp.shape[0]
        spec_concat_est, pcm_r, pcm_i, att_r, att_i, conv1_r_1, conv1_i_1, conv1_r_2, conv1_i_2 = sess.run([self.decode, self.pcmout_r, self.pcmout_i, self.attout_r, self.attout_i, self.conv1out_r_1, self.conv1out_i_1, self.conv1out_r_2, self.conv1out_i_2], feed_dict={self.data_input: inp})
        #spec_concat_est = sess.run(self.decode, feed_dict={self.data_input: inp})
        #pdb.set_trace() 

        dim = int(spec_concat_est.shape[-1] / 2)

        spec_est_half \
            = spec_concat_est[:, :, 0:dim] + 1j * spec_concat_est[:, :, dim:]

        enhanced = []
        for n in range(batch_size):
            tmp = ssp.spec_half2wav(spec_est_half[n],
                                    window_analysis,
                                    window_synthesis,
                                    length_wav[n], ratio)
            tmp_int = tmp * (2 ** 15)
            tmp_int = tmp_int.astype(np.int16)
            enhanced.append(tmp_int)
        return enhanced

    def run_decode_batch_step_notSC(self, sess, buf,
                              window_analysis,
                              window_synthesis,
                              length_wav, ratio):

        seqlen = self.get_length_np(buf)
        inp, mask = self.make_data_for_batch(buf, seqlen)
        batch_size = inp.shape[0]
        spec_concat_est = sess.run(self.decode,
                                   feed_dict={self.data_input: inp, self.bnorm: False})
        dim = int(spec_concat_est.shape[-1] / 2)

        spec_est_half \
            = spec_concat_est[:, :, 0:dim] + 1j * spec_concat_est[:, :, dim:]
        enhanced = []
        for n in range(batch_size):
            tmp = ssp.spec_half2wav(spec_est_half[n],
                                    window_analysis,
                                    window_synthesis,
                                    length_wav[n], ratio)
            tmp_int = tmp * (2 ** 15)
            tmp_int = tmp_int.astype(np.int16)
            enhanced.append(tmp_int)
        return enhanced

    def run_decode_batch_step_updown_target2(self, sess, buf,
                                     window_analysis,
                                     window_synthesis,
                                     length_wav, ratio):

        seqlen = self.get_length_np(buf)
        inp, mask = self.make_data_for_batch(buf, seqlen)
        batch_size = inp.shape[0]
        spec_concat_est = sess.run(self.decode,
                                   feed_dict={self.data_input: inp})

        spec_concat_est_real = spec_concat_est[:,:, 0:514]

        spec_concat_est_imag = spec_concat_est[:,:, 514:]

        # spec_est_mag = 20*np.log10(((spec_concat_est_real ** 2) +(spec_concat_est_imag ** 2) ) ** (0.5))
        # plt.imshow(np.squeeze(spec_est_mag), cmap='jet')
        # plt.colorbar()
        # plt.show()
        # exit()

        dim = int(spec_concat_est_real.shape[-1] / 2)

        spec_est_half_mic0 \
            = spec_concat_est_real[:, :, 0:dim] + 1j * spec_concat_est_imag[:, :, 0:dim]

        spec_est_half_mic1 \
            = spec_concat_est_real[:, :, dim:] + 1j * spec_concat_est_imag[:, :, dim:]


        enhanced = []
        for n in range(batch_size):

            tmp_0 = ssp.spec_half2wav(spec_est_half_mic0[n],
                                    window_analysis,
                                    window_synthesis,
                                    length_wav[n], ratio)

            tmp_1 = ssp.spec_half2wav(spec_est_half_mic1[n],
                                    window_analysis,
                                    window_synthesis,
                                    length_wav[n], ratio)

            tmp_int_0 = tmp_0*(2 ** 15)
            tmp_int_0 = tmp_int_0.astype(np.int16)

            tmp_int_1 = tmp_1 * (2 ** 15)
            tmp_int_1 = tmp_int_1.astype(np.int16)

            tmp_int = np.zeros((2, len(tmp_int_0)))
            tmp_int[0] = tmp_int_0
            tmp_int[1] = tmp_int_1

            enhanced.append(tmp_int.astype(np.int16))
        return enhanced


    def run_decode_batch_step_trial1(self, sess, buf,
                              window_analysis,
                              window_synthesis,
                              length_wav, ratio):

        seqlen = self.get_length_np(buf)
        inp, mask = self.make_data_for_batch(buf, seqlen)
        batch_size = inp.shape[0]
        spec_concat_est = sess.run(self.decode,
                                   feed_dict={self.data_input: inp})
        dim = int(spec_concat_est.shape[-1] / 2)
        numfrm_1 = int(spec_concat_est.shape[1] / 2)


        # separate mag and pha
        est_mag_2mic = spec_concat_est[:, :, 0:dim]
        est_pha_2mic = spec_concat_est[:, :, dim:]
        # separate mic0 and mic1
        est_mag_mic0 = est_mag_2mic[:, 0:numfrm_1, :]
        est_mag_mic1 = est_mag_2mic[:, numfrm_1:, :]
        est_pha_mic01 = est_pha_2mic[:, 0:numfrm_1, :]
        # corporate the 2 mic sample
        est_mag_mic_new = np.mean([est_mag_mic0, est_mag_mic1], axis=0)
        est_pha_mic_new = est_pha_mic01

        spec_est_half = est_mag_mic_new + 1j * est_pha_mic_new

        enhanced = []
        for n in range(batch_size):
            tmp = ssp.spec_half2wav(spec_est_half[n],
                                    window_analysis,
                                    window_synthesis,
                                    int(length_wav[n] / 2), ratio)
            tmp_int = tmp * (2 ** 15)
            tmp_int = tmp_int.astype(np.int16)
            enhanced.append(tmp_int)

        return enhanced

    def run_decode_batch_step_trial2(self, sess, buf):

        seqlen = self.get_length_np(buf)
        inp, mask = self.make_data_for_batch(buf, seqlen)
        batch_size = inp.shape[0]
        spec_concat_est = sess.run(self.decode,
                                   feed_dict={self.data_input: inp})
        dim = int(spec_concat_est.shape[-1] / 2)

        # separate real and imag
        est_real = spec_concat_est[:, :, 0:dim]
        est_imag = spec_concat_est[:, :, dim:]

        return est_real, est_imag, batch_size

    def enhancement_each_microphone(self, addr_in, net, shps, sess):

        # Perform enhancement.
        buf_mic0 = []
        buf_mic1 = []
        len_mic0 = []
        addr_noisy = addr_in
        fs, audio = wav.read(addr_noisy)

        # Separate the input data
        data_mic_0 = audio[:, 0]
        data_mic_1 = audio[:, 1]

        # mic_0 processing
        mic0_noisy = data_mic_0.astype(np.float32) / (2 ** 15)
        len_mic0.append(len(data_mic_0))
        logmag_noisy_mic0, pha_noisy_mic0 \
            = ssp.wav2logmagpha_half(mic0_noisy, shps.fs,
                                     shps.window_analysis,
                                     shps.frm_size, shps.ratio)
        bpd_noisy_mic0 = ssp.pha2bpd(pha_noisy_mic0, shps.fs, shps.frm_size, shps.ratio)
        bpd_noisy_mic0 = bpd_noisy_mic0[:, :128]
        tmpin = np.concatenate([logmag_noisy_mic0, pha_noisy_mic0, bpd_noisy_mic0], axis=1)
        buf_mic0.append(np.float16(tmpin))
        est_mag_mic0, est_pha_mic0, batch_size = net.run_decode_batch_step_trial2(sess, buf_mic0)

        # mic_1 processing
        mic1_noisy = data_mic_1.astype(np.float32) / (2 ** 15)
        logmag_noisy_mic1, pha_noisy_mic1 \
            = ssp.wav2logmagpha_half(mic1_noisy, shps.fs,
                                     shps.window_analysis,
                                     shps.frm_size, shps.ratio)
        bpd_noisy_mic1 = ssp.pha2bpd(pha_noisy_mic0, shps.fs, shps.frm_size, shps.ratio)
        bpd_noisy_mic1 = bpd_noisy_mic1[:, :128]
        tmpin = np.concatenate([logmag_noisy_mic1, pha_noisy_mic1, bpd_noisy_mic1], axis=1)
        buf_mic1.append(np.float16(tmpin))
        est_mag_mic1, est_pha_mic1, batch_size = net.run_decode_batch_step_trial2(sess, buf_mic1)

        est_mag_new = np.mean([est_mag_mic0, est_mag_mic1], axis=0)
        est_pha_new = est_pha_mic0

        spec_est_half = est_mag_new + 1j * est_pha_new

        enhanced = []
        for n in range(batch_size):
            tmp = ssp.spec_half2wav(spec_est_half[n],
                                    shps.window_analysis,
                                    shps.window_synthesis,
                                    len_mic0[0], shps.ratio)
            tmp_int = tmp * (2 ** 15)
            tmp_int = tmp_int.astype(np.int16)
            enhanced.append(tmp_int)
        return enhanced

    def make_data_for_batch(self, data, length):
        """Make zero-padded data and binary mask.
        """
        max_len = np.max(length)
        data1 = self.zero_padding(data, max_len)
        mask = self.length_vector2mask(length)
        return data1, mask

    @staticmethod
    def zero_padding(list_mat, max_frmlen):
        """Zero-pad batch data by maximum frame length of total data.

        Args:
            list_mat: batch data
            max_frmlen: maximum frame length of total data

        Returns:
            x_pad: zero-padded batch data
                   (batch size X maximum frame length X dim)
        """
        num_batches = len(list_mat)
        dim = list_mat[0].shape[1]
        x_pad = np.zeros((num_batches, max_frmlen, dim), dtype='float32')
        for batch_idx in range(num_batches):
            l_tmp = list_mat[batch_idx].shape[0]
            x_zero = np.zeros((max_frmlen - l_tmp, dim), dtype='float32')
            x_tmp = np.concatenate((list_mat[batch_idx], x_zero), 0)
            x_pad[batch_idx] = x_tmp
        return x_pad

    @staticmethod
    def length_vector2mask(length):
        """Convert length vector to binary mask.

        Args:
            length: length vector which has #frame of each utterance in batch
                    (# batches size by 1)

        Returns:
            mask: binary mask matrix (batch size X max length of length vector)
        """
        maxl = np.max(length)
        mask = np.ones((len(length), maxl))
        for idx in range(len(length)):
            mask[idx, length[idx]:] = 0
        for idx in range(len(length)):
            mask[idx, :] = mask[idx, :] / length[idx]
        return mask

    @staticmethod
    def convert_magpha2concatenated_spec(mag, pha):
        """Convert magnitude and phase to complex spectrum.

        Args:
            mag: magnitude
            pha: phase

        Returns:
            spec_concat: concatenated data [real part, imaginary part]
            spec_real: real part of complex data
            spec_imag: imaginary part of complex data
        """
        spec_real = mag * tf.cos(pha)
        spec_imag = mag * tf.sin(pha)
        spec_concat = tf.concat([spec_real, spec_imag], 2)
        return spec_concat, spec_real, spec_imag

    @staticmethod
    def get_length_np(data):
        """Get length of numpy list data

        Args:
            data: numpy list data

        Returns:
            data_length: length vector
        """
        num_samps = len(data)
        data_length = np.zeros(num_samps, dtype='int32')
        for i in range(num_samps):
            data_length[i] = np.int32(data[i].shape[0])
        return data_length
