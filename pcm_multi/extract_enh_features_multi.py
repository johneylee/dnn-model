...
def main():
    ...
    # wav_dir_name = '/home/leesunghyun/Downloads/enhancement_work/dnn_algorithm/PCM_multi/data/multi_RIR_random_rever/SH_format/train'
    # fea_dir_name = '/home/leesunghyun/Downloads/speech9/pcm_multi/workspace_my/features_updown_target2_small'
    # fea_name = 'train' # devel or train
    ...
    addr_list = scan_directory_multi_stereo('%s/noisy' % wav_dir_name)
    ...
    for idx_utt in range(num_utts):
        ...
        inp, ref = extract_features_multi(addr_noisy, addr_clean,
                                    featype_input_list,
    ...
    for path, dirs, files in os.walk(dir_name):
        for data in files:
            ...
            if ext == '.wav':
                ...
                dump = data.split('_')
                clean_name = dump[0] + '_' + dump[1] + '_' + dump[2] + '.wav'
                clean_path = path.replace('noisy', 'clean')
...
def extract_features_multi(addr_noisy, addr_clean,
    ...
    if len(mic0_noisy.shape) == 2:
        mic0_noisy = (mic0_noisy[:, 0] + mic0_noisy[:, 1]) * 0.5
    if len(mic1_noisy.shape) == 2:
        mic1_noisy = (mic1_noisy[:, 0] + mic1_noisy[:, 1]) * 0.5

    fs, audio_c = wav.read(addr_clean)
    wav_clean = audio_c.astype(np.float32) / (2 ** 15)
    if len(wav_clean.shape) == 2:
        wav_clean = (wav_clean[:, 0] + wav_clean[:, 1]) * 0.5

    mic0_noisy = mic0_noisy[:len(wav_clean)]
    mic1_noisy = mic1_noisy[:len(wav_clean)]
    wav_clean = wav_clean[:len(mic0_noisy)]

    wav_noise_mic0 = mic0_noisy - wav_clean
    wav_noise_mic1 = mic1_noisy - wav_clean

    vma = np.max(np.abs(wav_clean))
    wav_norm_clean = wav_clean / vma
    wav_norm_noise_mic0 = wav_noise_mic0 / vma
    wav_norm_noise_mic1 = wav_noise_mic1 / vma
    ...
    for feature_type in featype_refer_list:
        ...
        if feature_type == 'frm_rect_norm_clean': 
            frm_rect_norm_clean = ssp.enframe(wav_norm_clean, fs,
                                              np.ones(512),
                                              shps.frm_size, shps.ratio)
            frm_rect_norm_clean_old = frm_rect_norm_clean[:, :128]
            frm_rect_norm_clean = np.concatenate((frm_rect_norm_clean_old, frm_rect_norm_clean_old), axis=1)
        ...
        # Freq-domain clean
            ...
            mag_norm_clean, pha_clean \
                = ssp.wav2magpha_half(wav_norm_clean, fs, shps.window_analysis,
                                      shps.frm_size, shps.ratio)
            mag_norm_warp_clean_old = np.power(mag_norm_clean, 2.0 / 3.0)
            mag_norm_warp_clean = np.concatenate((mag_norm_warp_clean_old, mag_norm_warp_clean_old), axis=1)
        ...
        # Phase
            ...
            mag_clean, pha_clean_ \
                = ssp.wav2magpha_half(wav_clean, fs, shps.window_analysis,
                                      shps.frm_size, shps.ratio)
            pha_clean = np.concatenate((pha_clean_, pha_clean_), axis=1)
            ...
            mag_clean, pha_clean \
                = ssp.wav2magpha_half(wav_clean, fs, shps.window_analysis,
                                      shps.frm_size, shps.ratio)
            pha_clean = np.concatenate((pha_clean, pha_clean), axis=1)
