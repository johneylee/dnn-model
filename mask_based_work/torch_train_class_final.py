    ...
    def train(self):
        ...
        if self.network_name == 'dnn':
            self.model = dnn_sh(self.input_dim, self.num_context_window, self.hidden_dim, \
                                self.output_dim, self.num_layers)
        ...
        train_input = self.normalize_batch(self.train_input, mu_input, sig_input)
        devel_input = self.normalize_batch(self.devel_input, mu_input, sig_input)

        if self.network_name == 'dnn':
            # Make context window data
            print('Make context window data...')
            train_input = self.make_context_data(train_input, self.num_context_window)
            devel_input = self.make_context_data(devel_input, self.num_context_window)

        # Start training.
        self.num_utt_train = len(train_input)
        self.num_utt_devel = len(devel_input)
        self.idx_set = range(len(train_input))
        ...
        for i in range(self.training_epochs + 1):
            ...
            if self.network_name == 'lstm':
                for j in range(self.num_utt_train):  # all data from train_input
                    inp_t, ref_t = torch.Tensor(train_input[j]), torch.Tensor(self.train_refer[j])
            ...
            if self.network_name == 'dnn':
                ...
                for j in range(self.num_utt_train):  # all data from train_input
                    idxs = idx_set[j]
                    inp_t, ref_t = torch.Tensor(train_input[idxs]), torch.Tensor(self.train_refer[idxs])
                    self.optimizer.zero_grad()
                    est_mask = self.model(inp_t)
                    if self.mode == 'irm' or 'ibm':
                        mse_train_arr_ = self.cost_func(est_mask, ref_t)
            ...
            for j in range(self.num_utt_devel):
                inp_d, ref_d = torch.Tensor(devel_input[j]), torch.Tensor(self.devel_refer[j])
