...
"""lg data
    change (stft.npy)file to (.wav)file
"""
...
def perform_enhancement(addr_in, model_name, mode, shps):
    dir_name = os.path.dirname(addr_in)
    data_split = addr_in.split("/")[-1]
    data_part = data_split.split("_")
    addr_in_name = dir_name + "/" + data_part[1] + "_" + data_part[2] + ".wav"
    addr_out = addr_in_name.replace('.wav', '_' + model_name.split('/')[-1] + '.wav')
    print('%s' % addr_in)
    print(' -> %s' % addr_out)
...
for (path, dir, files) in os.walk("../data_lg/speech/test/noisy_beamforming"):
    for data in files:
        ext = os.path.splitext(data)[-1]
        # data_name = os.path.splitext(data)[0]
        # data_part = data_name.split("_")
        # data = data_part[1] + "_" + data_part[2] + ext
        if ext == '.wav':
