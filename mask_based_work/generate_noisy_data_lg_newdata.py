"""
example
python generate_noisy_data_test.py train
"""
import os
import sys
import numpy as np
...
mode = 'train'
speech_dir = '/home/leesunghyun/Downloads/LG_work/data/speech/'
noise_dir = '/home/leesunghyun/Downloads/LG_work/data/noise/'
# noise_dir = '../data/noise/' # original

snr_set = [-10, -5, 0, 5 ,10]
snr_set_string = ['m10dB', 'm5dB', '0dB', '5dB', '10dB']
num_snrs = len(snr_set)
noise_subset = ['noisex92_babble', 'noisex92_factory', 'aurora4_restaurant',\
				'noisex92_white', 'aurora4_exhibition', 'noisex92_military']
...
if os.path.isdir(save_dir + '/noisy') is False:
	...
	for n in range(9):
		os.system('mkdir' + save_dir + '/noisy{}'.format(n))
# Make lists for three mode
log_file_name = '/home/leesunghyun/Downloads/LG_work/data/log_generate_data_' + mode + '.txt'
...
if mode in ['train', 'devel']:
	...
	list_noise_files = sorted(glob(noise_dir + 'train' + os.sep + '*.wav'))
	...
	for n in range(9):
		...
		for addr_speech in list_speech_files:
			...
			# Write
			addr_noisy = speech_dir + mode + '/noisy{}'.format(n) + '/' + speech_file_name
	...
	list_noise_files = sorted(glob(noise_dir + 'test' + os.sep + '*.wav'))
	# list_noise_files = sorted(glob(noise_dir + 'noise*' + os.sep + '*.wav')) # original
	...
	for nidx in range(len(list_noise_files)):
		...
		for didx in range(len(snr_set)):
			...
			for addr_speech in list_speech_files:
				...
				# Write
				addr_noisy = speech_dir + mode + '/noisy/' + foldername + \
							 os.sep + snr_set_string[didx] + os.sep + speech_file_name
