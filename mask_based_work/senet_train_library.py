...
class SpeechEnhancementNetwork:
    ...
    def set_params(self, hps):
        ...
        self.dir_to_save = hps.job_dir + '_%s' % hps.mode   # ex) job_dir = './job/samples_irm' , mode = 'irm'
        create_folder(self.dir_to_save)                     # ex) dir_to_save = 'samples_irm_irm'

    def calculate_total_params(self):
        # Calculate the size of total network.
        self.total_params = 0
        for variable in tf.trainable_variables(): # trainable_variables(): add new variable
    ...
    def initialize_model(self, sess):
        ...
        self.create_placeholder()       # Create input, hidden layer, output dim.
        self.define_layers()            # Define weight, bias

        # Select mode.
        if self.hps.mode == 'spectrum':
            self.define_spec_network
        elif self.hps.mode == 'direct':
    ...
    def create_placeholder(self):
        # Creat placeholder.
        self.data_input \
            = tf.placeholder(tf.float32, shape=[None, self.hps.dnn_struct[0]])  # [0]: input dim.
        self.data_refer \
            = tf.placeholder(tf.float32, shape=[None, self.hps.dnn_struct[-1]])  # [-1]: output dim.


    def define_layers(self):  # weight, bias define
        # Define DNN layers.
        nodes = self.hps.dnn_struct     # 1285-512-512-512-257 <<  from get_structure_params << from tools_for_network
        self.weight = []
        self.bias = []
        with tf.name_scope("DNN"):
            for i in range(1, len(nodes)):    # W1, W2, ... W(number of nodes)
                W = tf.get_variable("W%d" % i, shape=[nodes[i-1],nodes[i]], initializer=tf.contrib.layers.xavier_initializer())
                #  W1 = [1285,512], W2 = [512,512], W3 = [512,512], W4 = [512,512], W5 = [512,257]
                self.weight.append(W)
                
                B = tf.Variable(tf.random_normal([nodes[i]]))
                self.bias.append(B)

    def feed_layers(self, data_input):      # construct for input layer, connect activation func. other layer
        # Connect DNN layers.
    ...
    def define_spec_network(self):               # output layer activation func.
                                                # define loss compute type: mean squre error
        # Define mse (enhanced_mag - clean_mag)
        ...
        # data refer: clean log magnitude - 그니까, 피쳐 뽑을때 레퍼런스로 clean log mag 을 뽑으셈
        loss = tf.reduce_mean(tf.square(enhanced_mag - self.data_refer))

        self.error = loss
        # Define optimizer.
        self.define_optimization(loss)
        self.decode = tf.cast(enhanced_mag, dtype=tf.float32)       # tf.cast
    ...
    def define_irm_network(self):               # output layer activation func.
                                                # define loss compute type: mean squre error
        # Define output function and loss
        last_hidden_layer = self.feed_layers(self.data_input)
        est_mask = tf.sigmoid(tf.add(tf.matmul(last_hidden_layer, self.weight[-1]), self.bias[-1]))
        ...
        # Define optimizer.
        self.define_optimization(loss)
        self.decode = tf.cast(est_mask, dtype=tf.float32)       # tf.cast
    ...
    def train(self, sess, train_input, train_refer, devel_input, devel_refer):
        ...
        for i in range(self.hps.max_epochs + 1):
            ...
            # Train data.
            if i > 0:
                idx_set = np.random.permutation(idx_set)        # data 랜덤으로 섞기
