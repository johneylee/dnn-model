...
def get_structure_params(input_dim=257, output_dim=257, layer_size=512, 
    ...
    dnn_struct = [input_dim * (num_context_window * 2 + 1)]     # 257x5 = 1285 : input dim.
    for i in range(num_layer):                                  # layer 3 각각에
        dnn_struct.append(layer_size)                           # 1285-512-512-512 : + hidden layer dim.
    dnn_struct.append(output_dim)                               # 1285-512-512-512-257 : + output dim.
    return dnn_struct
...
def denormalize_batch(data, mu, sigma):
    # mean, variance -> size 맞춰주기
    num_samps = len(data)
    for i in range(num_samps):
        data[i] = denormalize(data[i], mu, sigma)
    return data
